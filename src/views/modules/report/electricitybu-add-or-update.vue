<template>
  <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible" width="30%">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="100px">
      <el-form-item label="电表型号" prop="counterId">
        <el-select v-model="dataForm.counterId" style="width:100%" placeholder="电表型号" filterable>
          <el-option v-for="op in counterIdOptions" :key="op.item" :label="op.value" :value="op.item" />
        </el-select>
      </el-form-item>
      <el-form-item label="电表名称" prop="counterName">
        <el-input v-model="dataForm.counterName" placeholder="电表名称"></el-input>
      </el-form-item>
      ...
      <el-form-item label="电量占比(%)" prop="buRatio">
        <el-input-number v-model="dataForm.buRatio" placeholder="电量占比" controls-position="right" :precision="2" :min="0.01" :max="100">
          <el-button slot="prepend">%</el-button>
        </el-input-number>
      </el-form-item>
      <el-form-item label="用电系数" prop="counterRatio">
        <el-input-number v-model="dataForm.counterRatio" placeholder="用电系数" controls-position="right" :precision="1" :min="1" :max="9999"/>
      </el-form-item>
      <!--
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        counterRatio: [
          { required: true, message: '用电系数不能为空', trigger: 'blur' }
        ],
        // attr3: [
  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/report/electricitybu/info/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.dataForm.counterId = data.electricityBu.counterId
              this.dataForm.counterName = data.electricityBu.counterName
              this.dataForm.counterType = data.electricityBu.counterType
              this.dataForm.buName = data.electricityBu.buName
              this.dataForm.positionName = data.electricityBu.positionName
              this.dataForm.buRatio = data.electricityBu.buRatio
              this.dataForm.counterRatio = data.electricityBu.counterRatio
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/report/electricitybu/${!this.dataForm.id ? 'save' : 'update'}`),
            method: 'post',
            data: this.$http.adornData({
              ...
              'counterRatio': this.dataForm.counterRatio,

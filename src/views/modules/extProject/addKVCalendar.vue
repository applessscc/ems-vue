<template>
    <div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center;margin: 0px 20px;">
            <div style="size: 10px; ">
                {{ calendarHeaderText }}
            </div>
            <div>
                <el-button-group>
                    <el-button @click="goToPreviousWeek" size="mini">上周</el-button>
                    <el-button @click="goToCurrentWeek" size="mini">当周</el-button>
                    <el-button @click="goToNextWeek" size="mini">下周</el-button>
                </el-button-group>
            </div>
        </div>
...
export default {
    ...
    methods: {
        getWeekOfMonth(date) {
            ...
            const offset = (dayOfWeek === 0 ? 7 : dayOfWeek) - 1; // 以周一为一周开始
        ...
        getMonthRange() {
            const now = new Date();
            now.setMonth(now.getMonth() + this.monthOffset); // 根据偏移调整月份

...
export default {
  ...
  methods: {
    ...
    handleDateClick(date) {
      ...
      const isExisting = this.workDateList.some(d => d.toISOString().split('T')[0] === dateString);

      if (!isExisting) {
        // 使用 Vue.set 确保响应式更新
        this.$set(this.workDateList, this.workDateList.length, date); // push 操作
      } else {
        this.workDateList = this.workDateList.filter(d => d.toISOString().split('T')[0] !== dateString); // Remove the date if it's already in the list
      }
    }
    ,

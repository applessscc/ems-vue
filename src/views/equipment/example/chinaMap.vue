    ...
    initCharts() {
        ...
        tooltip: {
          ...
          formatter: (params) => {
            return `<p style="text-align:center;line-height: 30px;height:30px;font-size: 14px;border-bottom: 1px solid #7A8698;">${params.name}</p>
              <div style="line-height:22px;margin-top:5px">GDP<span style="margin-left:12px;color:#fff;float:right">${(params.data && params.data.value) || "--"}</span></div>`;
          },
        },
        // 地图配置文字显示
        geo: {
      ...
      this.charts.on("click", (dom) => {
        ...
        if (dom.componentType === "series") {
          code = data && data.areaUniCode ? data && data.areaUniCode : this.lastCity[name];
        } else {
